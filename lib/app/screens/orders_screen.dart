...
class _OrdersScreenState extends State<OrdersScreen> {
        ...
        children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              color: ColorHelper.blue500.color.withOpacity(0.1),
              border: Border(bottom: BorderSide(color: Colors.grey.withOpacity(0.2))),
            ),
            child: Row(
              children: [
                Icon(Icons.info_outline, color: ColorHelper.white.color, size: 16),
                const SizedBox(width: 8),
                Expanded(
                  child: Text('Demo Mode: Click on orders to update status or view details', style: TextStyle(fontSize: 12, color: ColorHelper.blue500.color)),
                ),
                ElevatedButton.icon(
          ...
          Consumer<OrderProvider>(
            builder: (context, orderProvider, child) {
              return Container(
                ...
                child: Row(
                  children: [
                    _buildQuickStat('Total', orderProvider.orders.length.toString(), ColorHelper.blue500.color),
                    const SizedBox(width: 8),
                    _buildQuickStat('Pending', orderProvider.pendingOrders.length.toString(), ColorHelper.yellow500.color),
                    const SizedBox(width: 8),
                    _buildQuickStat('Delivered', orderProvider.deliveredOrders.length.toString(), ColorHelper.green500.color),
                    const SizedBox(width: 8),
                    _buildQuickStat('Revenue', '\$${orderProvider.totalRevenue.toStringAsFixed(0)}', ColorHelper.blue500.color),
                  ],
                ),
              );
            },
            ...
            child: Consumer<OrderProvider>(
              builder: (context, orderProvider, child) {
                ...
                return RefreshIndicator(
                  ...
                  child: _selectedView == 'list' ? _buildListView(orders) : _buildGridView(orders),
                );
              },
            ),
          ),
  ...
  Widget _buildOrderCard(Order order, {required bool isCompact}) {
    ...
    final Color avatarBg = ColorHelper.blue500.color;
                ...
                Expanded(
                  child: OutlinedButton(
                    ...
                    style: OutlinedButton.styleFrom(
                      foregroundColor: ColorHelper.blue500.color,
                      side: BorderSide(color: ColorHelper.blue500.color.withOpacity(0.25)),
                    ...
                    child: Text(
                      'See Details',
                      style: TextStyle(fontSize: 13, fontFamily: 'Poppins', color: ColorHelper.blue500.color),
                    ),
                  ),
                ),
                const SizedBox(width: 12),
                Expanded(
                  child: ElevatedButton(
                    ...
                    style: ElevatedButton.styleFrom(
                      backgroundColor: ColorHelper.yellow500.color,
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            _buildStatusOption(order, 'pending', 'Pending', Icons.schedule, ColorHelper.yellow500.color),
            _buildStatusOption(order, 'confirmed', 'Confirmed', Icons.check_circle, ColorHelper.blue500.color),
            _buildStatusOption(order, 'processing', 'Processing', Icons.build, ColorHelper.blue500.color),
            _buildStatusOption(order, 'shipped', 'Shipped', Icons.local_shipping, ColorHelper.green500.color),
            _buildStatusOption(order, 'delivered', 'Delivered', Icons.done_all, ColorHelper.green500.color),
            _buildStatusOption(order, 'cancelled', 'Cancelled', Icons.cancel, ColorHelper.red500.color),
          ],
        ),
      ),
    );
  ...
  void _createDemoOrder() async {
    ...
    if (success && mounted) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: const Text('Demo order created successfully!'), backgroundColor: ColorHelper.green500.color));
    }
  }
  ...
  Future<void> _updateOrderStatus(String orderId, String newStatus) async {
    ...
    if (success && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Order status updated to $newStatus'), backgroundColor: ColorHelper.green500.color));
    }
  }
}
